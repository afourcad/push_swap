...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/02/27 17:15:33 by afourcad          #+#    #+#             */
/*   Updated: 2017/03/27 18:10:44 by afourcad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_write_dec_with_flags(char *buff, t_arg *param, char *nbr, int neg)
{
	...
	{
		if (param->is_width == TRUE && param->is_pre == TRUE)
			buff = ft_width_and_pre(buff, param, nbr, neg);
		else if (param->is_width == TRUE && param->is_pre != TRUE)
...
char	*ft_decimal_else(char *buff, char *nbr, t_arg *param)
{
	nbr = ft_uitoa(param->type.my_uint);
	buff = ft_write_dec_with_flags(buff, param, nbr, FALSE);
	free(nbr);
	return (buff);
}
...
char	*ft_decimal(char *buff, t_arg *param)
{
	...
	if (param->type.my_int == (intmax_t) - 9223372036854775808ULL)
	{
		...
		buff = ft_write_dec_with_flags(buff, param, nbr, is_neg);
			...
			|| param->specifier == 'D')
	{
		is_neg = param->type.my_int < 0 ? TRUE : FALSE;
		...
		buff = ft_decimal_else(buff, nbr, param);
	return (buff);
}
